...
const AdminApproveUsers = () => {
  ...
  useEffect(() => {
    ...
    const fetchPendingUsers = async () => {
      ...
      try {
        ...
        console.error(
          "Error al obtener los usuarios pendientes:",
          error.message
        );
      }
    };
    ...
    const fetchApprovedUsers = async () => {
      ...
      try {
        ...
        console.error(
          "Error al obtener los usuarios aprobados:",
          error.message
        );
      }
    };
  ...
  const handleDeny = async (user) => {
    ...
    try {
      ...
      console.log(
        `Usuario con email ${user.email} ha sido denegado y eliminado de pendingUsers.`
      );
    } catch (error) {
  ...
  const handleApprove = async (user) => {
    try {
      ...
      if (result) {
        setPendingUsers(pendingUsers.filter((u) => u.id !== user.id));
        console.log(
          `Usuario con email ${user.email} ha sido aprobado y movido a approvedUsers.`
        );
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto max-w-md mt-8">
          ...
          {pendingUsers.map((user) => (
            ...
            >
              <div>
                ...
                <p>
                  Solicitado en: {user.requestedAt.toDate().toLocaleString()}
                </p>
              </div>
      ...
      <h2 className="text-3xl font-bold text-center mb-6 mt-12">
        Usuarios aprobados
      </h2>
